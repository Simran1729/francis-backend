  ...
  // const teamId = teamIdMap[team] || "";
      ...
      // console.log("Team id : ", teamId);
        ...
        const ticketData = {
          ...
          contactId: "1142108000000477001", // Set correct contact ID
...
app.post("/test/create-ticket", upload.array('files', 10), async (req, res) => {
  try{
      // Step 1: Create ticket in Zoho Desk
      const accessToken = await fetchAccessToken(); // Replace with your token
      if(!accessToken){
      ...
      // console.log("Team id : ", teamId);
        ...
        //FOR AMIT TESTING - COMMENT THIS OUT
      //   const ticketData = {
      //     subject: subject,
      //     departmentId: departmentId, // Zoho department ID (keep this same)
      //     description: `${description}`, // Merging description & notes
      ...
      const ticketData = {
        ...
        description: "test description", // Merging description & notes
        language: "English",
        status: "Open", // Setting initial status
        category: "general", // Adjust category if needed
        contactId: "1142108000000477001", // Set correct contact ID
        productId: "", // Can be updated if needed,
        teamId : "1142108000000533040",
        priority : "Low",
        cf: { // ✅ Add custom fields (cf)
            cf_permanentaddress: null,
            cf_dateofpurchase: null,
...
///////////////////////////////////////////////////////----------------------//////////////////////////////
app.post("/api/wd/create-ticket1", upload.array('files', 10), async (req, res) => {
  try {
      // Extract form data
      const { subject, departmentId, description, severity, contactId, ticketCreator, team, projectCode, priority } = req.body;

      const MAX_FILE_SIZE = 15 * 1024 * 1024; // 20MB limit
      ...
      if (!accessToken) {
        ...
        const teamId = teamIdMap[team] || "";  // Use team map to fetch teamId
      ...
      const ticketId = ticketResponse.data.id; // Get the ticket ID
      const ticketNumber = ticketResponse.data.ticketNumber; // Get the ticket number

      let attachmentUploaded = false;
      let workDriveUrlUpdated = false;
      let workDriveFileUrl = null;
      ...
      if (req.files && req.files.length > 0) {
          for (const file of req.files) {
              ...
              if (file.size <= MAX_FILE_SIZE) {
                  // Step 2A: Attempt to upload directly to Zoho Desk
                  const formData = new FormData();
                  formData.append("file", file.buffer, { filename: file.originalname });
                  ...
                  try {
                      ...
                      console.log(`✅ Attachment uploaded to ticket: ${file.originalname}`);
                      attachmentUploaded = true; // Mark attachment as uploaded
                  } catch (error) {
                      console.error(`❌ Failed to upload attachment directly. Falling back to WorkDrive.`);
                  }
              }

              if (!attachmentUploaded) {
                  // Step 2B: Fallback - Upload to Zoho WorkDrive
                  const workDriveUrl = await uploadToWorkDrive(file.buffer, file.originalname, accessToken);

                  if (workDriveUrl) {
                      console.log(`✅ File uploaded to Zoho WorkDrive: ${file.originalname}`);
                      workDriveUrlUpdated = true; // Mark the URL update as successful
                      workDriveFileUrl = workDriveUrl;
                  }
              }
          }
      }

      // Step 3: Update ticket with WorkDrive URL (if applicable)
      if (workDriveUrlUpdated && workDriveFileUrl) {
          await axios.put(
              `https://desk.zoho.com/api/v1/tickets/${ticketId}`,
              {
                  data: {
                      cf: { "cf_attachment_url": workDriveFileUrl } // Update custom field with WorkDrive URL
                  }
              },
              {
                  headers: {
          ...
          console.log(`✅ WorkDrive URL updated in ticket: ${workDriveFileUrl}`);
      }
...
// Helper function to upload file to Zoho WorkDrive
const uploadToWorkDrive = async (fileBuffer, fileName, accessToken) => {
    try {
        const formData = new FormData();
        formData.append("file", fileBuffer, { filename: fileName });

        // Zoho WorkDrive API URL for file upload
        const url = `https://workdrive.zoho.com/api/v1/files/upload?parent_id=kat7c20f29313735d4b12adabeb4086cfd5ee`;
        ...
        const fileUrl = response.data.data.link; // Assuming this is where the WorkDrive file URL is returned
        return fileUrl;
    } catch (error) {
        console.error("Error uploading to Zoho WorkDrive:", error.message);
